...
private enum CardScanParser {
    ...
    static func isValidPlayerFullName(_ s: String, maxWords: Int) -> Bool {
        ...
        if t.count < 5 || t.count > 40 { return false }
        ...
        let banned = [
            "command","upper","deck","trading","card","collection","series","rookie","rookies",
            "authentic","autograph","patch","hockey","nhl","league","national","www","http",
            "blackhawks","blackhawk","congratulations","received","signature"
        ]
        ...
        // ✅ On accepte ALL-CAPS si ça ressemble à un nom (ex: "FRANK NAZAR")
        let allCaps = words.allSatisfy { $0 == $0.uppercased() }
        if allCaps {
            // 2-3 mots, chaque mot 3-12 lettres, et aucun mot n'est un stopword évident
            if words.count > 3 { return false }
            for w in words {
                let lw = w.lowercased()
                if lw.count < 3 || lw.count > 12 { return false }
                if ["upper","deck","authentic","rookie","rookies","blackhawks","nhl"].contains(lw) { return false }
            }
            return true
        }

        // sinon, rejette une ligne qui contient des mots tout en MAJUSCULES (souvent du bruit)
