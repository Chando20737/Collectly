...
extension EbayBrowseService {

    /// Alias used by some parts of the app (and earlier iterations).
    /// Internally calls `searchActiveListings(query:limit:token:)`.
    func searchItemSummaries(query: String, limit: Int = 20, token: String) async throws -> [ItemSummary] {
        try await searchActiveListings(query: query, limit: limit, token: token)
    }
}
