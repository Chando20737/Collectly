...
struct ProfileView: View {
    ...
    // Compteur de scans Firebase
    @State private var firebaseScansCount: Int? = nil
    @State private var isLoadingScansCount: Bool = false
            ...
            Form {
                ...
                Section("Statistiques") {
                    NavigationLink {
                        ...
                        Label("Stats eBay", systemImage: "chart.bar.fill")
                    }
                    ...
                    NavigationLink {
                        ...
                        HStack {
                            Label("Scans Firebase", systemImage: "cloud.fill")
                            Spacer()
                            if isLoadingScansCount {
                                ProgressView()
                                    .scaleEffect(0.8)
                            } else if let count = firebaseScansCount {
                                Text("\(count)")
                                    .foregroundStyle(.secondary)
                            }
                        }
    ...
    private func loadProfile() {
        ...
        // Charger le compte de scans Firebase
        Task {
            await loadFirebaseScansCount()
        }
    }
    ...
    // MARK: - Load Firebase Scans Count
    
    @MainActor
    private func loadFirebaseScansCount() async {
        guard let uid = session.user?.uid else {
            print("⚠️ Firebase scans count: No user logged in")
            firebaseScansCount = nil
            return
        }
        
        print("🔍 Loading Firebase scans count (ALL scans)")
        isLoadingScansCount = true
        defer { isLoadingScansCount = false }
        
        do {
            // TEMPORAIRE: Compter TOUS les scans (sans filtre)
            let snapshot = try await db.collection("scans")
                .getDocuments()
            
            firebaseScansCount = snapshot.documents.count
            print("✅ Total scans in collection: \(snapshot.documents.count)")
            
            // Debug: Afficher les champs du premier document
            if let firstDoc = snapshot.documents.first {
                print("📋 First scan fields: \(firstDoc.data().keys.joined(separator: ", "))")
            }
        } catch {
            print("❌ Error loading Firebase scans count: \(error)")
            firebaseScansCount = nil
        }
    }
